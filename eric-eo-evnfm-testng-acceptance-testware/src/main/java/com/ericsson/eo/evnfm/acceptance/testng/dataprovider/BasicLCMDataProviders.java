...
import static com.ericsson.eo.evnfm.acceptance.testng.dataprovider.Constants.CNFS_TO_UPGRADE;
...
public class BasicLCMDataProviders {
    private static final String BASIC_LCM_DATA_PARAMETER = "basicLCMData";
    private static final String DEFAULT_BASIC_LCM_DATA_PATH = "basicLCM/basicLCM.yaml";
    ...
    public static List<EvnfmCnf> loadSuccessfulUpgradeConfigData(ITestContext iTestContext) throws IOException {
        String dataFilename = resolveConfig(iTestContext);
        List<EvnfmCnf> cnfsToUpgradeSuccessful = loadYamlConfiguration(dataFilename, CNFS_TO_UPGRADE, new TypeReference<>() {
        });
        cnfsToUpgradeSuccessful.forEach(TestExecutionGlobalConfig::addFlowSuffix);

        return cnfsToUpgradeSuccessful;
    }
    ...
    @DataProvider(parallel = true)
    public Object[] getInstancesToUpgradeSuccess(ITestContext iTestContext) throws IOException {
        return loadSuccessfulUpgradeConfigData(iTestContext).toArray();
    }
}
